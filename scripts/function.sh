...
get_arch_name() {
  ...
  5) echo "arm64" ;; # ios, tvos, macos, xros
  6) echo "arm64e" ;; # ios
  7) echo "i386" ;; # ios
  8) echo "x86" ;; # android
  9) echo "x86-64" ;; # android, ios, linux, macos, tvos, xros
  10) echo "x86-64-mac-catalyst" ;; # ios
  11) echo "arm64-mac-catalyst" ;; # ios
  12) echo "arm64-simulator" ;; # ios, tvos, xros
  esac
}
...
from_arch_name() {
  ...
  arm64) echo 5 ;; # ios, tvos, macos, xros
  arm64e) echo 6 ;; # ios
  i386) echo 7 ;; # ios
  x86) echo 8 ;; # android
  x86-64) echo 9 ;; # android, ios, linux, macos, tvos, xros
  x86-64-mac-catalyst) echo 10 ;; # ios
  arm64-mac-catalyst) echo 11 ;; # ios
  arm64-simulator) echo 12 ;; # ios, xros
  esac
}
...
is_library_supported_on_platform() {
  ...
  53 | 54 | 55 | 57 | 58)
    if [[ ${FFMPEG_KIT_BUILD_TYPE} == "ios" ]] || [[ ${FFMPEG_KIT_BUILD_TYPE} == "tvos" ]] || [[ ${FFMPEG_KIT_BUILD_TYPE} == "macos" ]] || [[ ${FFMPEG_KIT_BUILD_TYPE} == "xros" ]]; then
      echo "0"
    else
      echo "1"
    fi
...
set_virtual_library() {
  case $1 in
  libiconv)
    if [[ ${FFMPEG_KIT_BUILD_TYPE} == "ios" ]] || [[ ${FFMPEG_KIT_BUILD_TYPE} == "tvos" ]] || [[ ${FFMPEG_KIT_BUILD_TYPE} == "macos" ]] || [[ ${FFMPEG_KIT_BUILD_TYPE} == "xros" ]] || [[ ${FFMPEG_KIT_BUILD_TYPE} == "apple" ]]; then
      ENABLED_LIBRARIES[LIBRARY_APPLE_LIBICONV]=$2
    else
      ENABLED_LIBRARIES[LIBRARY_LIBICONV]=$2
    fi
