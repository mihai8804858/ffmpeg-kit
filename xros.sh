...
echo -e "\nBuilding ffmpeg-kit ${BUILD_TYPE_ID}shared library for xrOS\n"
echo -e -n "INFO: Building ffmpeg-kit ${BUILD_VERSION} ${BUILD_TYPE_ID}for xrOS: " 1>>"${BASEDIR}"/build.log 2>&1
echo -e "$(date)\n" 1>>"${BASEDIR}"/build.log 2>&1
...
if [[ ${NO_FRAMEWORK} -ne 1 ]]; then
  ...
  echo -e "INFO: Skipped creating xrOS frameworks.\n" 1>>"${BASEDIR}"/build.log 2>&1
fi
